...
public class CategoriaDao extends Conexion<Categoria> {
	...
	public Categoria obtener(Categoria e) throws Exception {
		...
		try {
			cn = obtenerConexion();
			String sql = "SELECT * FROM CATEGORIA WHERE idCategoria=? and habilitado=1 ORDER BY nombre";
			pr = cn.prepareStatement(sql);
			pr.setInt(1, e.getIdCategoria());
			rs = pr.executeQuery();
			while (rs.next()) {
	...
	public List<Categoria> listar() throws Exception {
		...
		try {
			cn = obtenerConexion();
			String sql = "SELECT * FROM CATEGORIA WHERE habilitado=1 ORDER BY nombre";
			pr = cn.prepareStatement(sql);
			rs = pr.executeQuery();
			while (rs.next()) {
				categoria = new Categoria();
	...
	public List<Categoria> listar(String nombre) throws Exception {
		...
		try {
			...
			sql += " WHERE UCASE(nombre) LIKE '%" + nombre + "%' and habilitado=1 ";
			sql += " ORDER BY nombre";
			pr = cn.prepareStatement(sql);
			rs = pr.executeQuery();
			while (rs.next()) {
