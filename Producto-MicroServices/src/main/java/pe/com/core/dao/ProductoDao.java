...
public class ProductoDao extends Conexion<Producto> {
    ...
    public Producto obtener(Producto e) throws Exception {
        ...
        try{
            cn = obtenerConexion();
            String sql = "select p.*, c.nombre as categoria from producto p inner join categoria c on p.idCategoria = c.idCategoria WHERE p.idProducto=? and habilitado=1 ORDER BY nombre";
            pr = cn.prepareStatement(sql);
            pr.setInt(1, e.getIdProducto());
            rs = pr.executeQuery();
            while(rs.next()){
    ...
    public List<Producto> listar() throws Exception {
       List<Producto> productos = new ArrayList<Producto>();
        ...
        try{
            cn = obtenerConexion();
            String sql = "select p.*, c.nombre as categoria from producto p inner join categoria c on p.idCategoria = c.idCategoria WHERE p.habilitado=1 ORDER BY nombre";
            pr = cn.prepareStatement(sql);
            rs = pr.executeQuery();
            while(rs.next()){
                producto = new Producto();
    ...
    public List<Producto> listar(String nombre) throws Exception {
        List<Producto> productos = new ArrayList<Producto>();
         ...
         try{
             ...
             sql+=" WHERE UCASE(p.nombre) LIKE '%" + nombre + "%' and p.habilitado=1" ;
             sql+=" ORDER BY nombre";
             pr = cn.prepareStatement(sql);
             rs = pr.executeQuery();
             while(rs.next()){
